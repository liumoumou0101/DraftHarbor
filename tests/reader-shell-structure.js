...
const readerIllustrations = read('src/desktop/shell/reader-illustrations.js');
const readerIllustrationCss = read('src/styles/desktop/reader-illustrations.css');
const readerHud = read('src/desktop/shell/reader-hud.js');
const readerAnnotationUi = read('src/desktop/shell/reader-annotation-ui.js');
const readerBookmarks = read('src/desktop/shell/reader-bookmarks.js');
...
assert.ok(desktop.includes('src/desktop/shell/reader-illustrations.js'), 'reader illustration module must load independently');
assert.ok(desktop.includes('src/styles/desktop/reader-illustrations.css'), 'reader illustration styles must load independently');
assert.ok(readerIllustrations.includes('readerIllustrationDropzone'), 'illustrated mode must expose a per-page image dropzone');
assert.ok(readerIllustrations.includes("addEventListener('drop'"), 'illustrated mode must accept files dropped on the current page');
assert.ok(readerIllustrationCss.includes('.desktop-reader-illustration-dropzone'), 'the per-page image dropzone must remain visible beside active images');
assert.ok(readerIllustrations.includes('readerIllustrationMutationAllowed'), 'illustration writes must share a focus-mode read-only guard');
assert.ok(readerIllustrationCss.includes('[data-reader-focus-mode="true"] .desktop-reader-illustration-dropzone'), 'focus mode must hide the illustration dropzone');
assert.ok(readerHud.includes('syncReaderIllustrationControls'), 'focus-mode transitions must resync illustration write controls');
assert.ok(readerAnnotationUi.includes('if (readerState.focusMode) return;'), 'focus mode must reject annotation mutations');
assert.ok(readerBookmarks.includes('if (readerState.focusMode) return;'), 'focus mode must reject bookmark mutations');
