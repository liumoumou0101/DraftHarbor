...
(async () => {
  ...
  assert.strictEqual(zenMiniMax.transport, 'anthropic-messages');
  assert.ok(ModelCatalog.isOpencodeGatewayCallable(zenMiniMax));
  ...
  assert.strictEqual(goCatalog.models.find((item) => item.id === 'glm-5.2').thinkingControl, 'always-on');
  assert.strictEqual(goCatalog.models.find((item) => item.id === 'glm-5.2').thinkingSupported, false);
  assert.strictEqual(presented.models.find((item) => item.id === 'glm-5.2').thinkingSupported, true);
  assert.strictEqual(goCatalog.models.find((item) => item.id === 'qwen3.8-flash').thinkingControl, 'toggle');
  assert.strictEqual(goCatalog.models.find((item) => item.id === 'mimo-v2.5-pro').thinkingSupported, true);
