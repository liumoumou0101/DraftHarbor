...
(async () => {
  ...
  let browser = null;
  try {
    await projectService.createProject(dataRoot, { id: 'creation-ui-project', title: '从零创作界面验收' });
    ...
    await page.waitForFunction(() => document.querySelector('[data-native-project-title]').textContent.includes('从零创作界面验收'));
    ...
    await generateAndApprove(page, '故事蓝图与冲突结构');
    ...
    const opened = await projectService.openProject(dataRoot, 'creation-ui-project');
    ...
    const compendiumResponse = await fetch(`${servers.appUrl}/api/compendium?projectId=creation-ui-project`);
