...
function projectToLegacySnapshot(project) {
  ...
  const chapters = (project.chapters || []).map((chapter) => ({
    ...
    summaryUpdated: chapter.summaryUpdated || '',
    summarySource: chapter.summarySource || '',
    summaryStale: !!chapter.summaryStale,
  ...
  const scenes = (project.scenes || []).map((scene) => ({
    ...
    summaryUpdated: scene.summaryUpdated || '',
    summarySource: scene.summarySource || '',
    summaryStale: !!scene.summaryStale,
    order: scene.order || 0,
