...
| 编号 | 功能 | 状态 | 依赖 | 完成标准 | 下一步 |
| --- | --- | --- | --- | --- | --- |
| F-09 | 半自动小说工作流 | 已完成；F-09.6 二十万字真实 Provider 验收已跑通，进入质量缺陷收敛 | 写作模块、资料库、上下文解析、AI 任务与 Provider、工作流存储、备份恢复 | 续写、从零创作和大段重写闭环通过真实 Provider 验收；写作、资料库和工作流双向转交；长篇生成具备细纲确认、版本、锁、恢复、幂等写回和可视化画布 | 2026-07-29 已生成 209,052 字/25 场景/6 批；下一步修复边界重叠、元数据泄漏、技术说明化和批次分章 |
...
- [ ] 将可见质量指标编译进现有“锁”机制，而不是另建一套互不相认的门禁：对话比例、技术说明腔、计划结果兑现、重复句式和伏笔状态均显示目标、实际、证据、作用范围与严重度。
- [ ] 质量锁默认采用“可见软锁”：提示偏差但不机械阻断；用户明确锁定的质量要求、已批准场景计划中的必达结果和现有事实/排除硬锁才可升级为阻断。用户可逐项豁免、调整或发起定向重写。
- [ ] 扩展 `workflow-writing-instructions@1` 的可选质量目标：目标对话区间、禁止/慎用词汇、解释性旁白上限、重复句式、允许的专业语体和需要持续追踪的伏笔；旧 Revision 继续兼容。
- [ ] 在自由文本指令之外提供可见的结构化设置，但结构化设置只是用户要求的编译结果，用户可编辑、关闭或恢复旧版本。
- [ ] 每场保存后计算轻量指标：对话比例、段落/句子长度、重复短语、重复句式、用户禁用词和创作过程标签；指标进入 Revision 元数据或独立审查产物，不复制正文。
- [ ] 批次审查比较“本批实际指标”和“用户目标”，展示偏差与证据。对话目标按整批和场景类型综合判断，动作场景、独处场景不得被机械强塞对话。
- [ ] 为本次黑童话测试增加项目级“避免现代技术说明腔”规则样本，识别赫兹、精确生理参数、神经解剖式解释等文体漂移；科幻、医学等题材只有用户启用该规则时才检查。
- [ ] 把“不是……是……”“不是比喻”等高频句式作为软告警并显示分布，不做全局字符串删除；允许用户选择若干场景发起针对性重写。
- [ ] 真实两万字流式样本补充缺陷：实际对话比例 19.31%，第四场出现“自动生成约束系统检测”等技术说明腔；当前确定性审查未检出技术说明腔，并把已明确发生的主动交易误报为 `direction_missing`。质量指标必须提供证据并降低纯字面约束检查的权重。
- [ ] 未解线索进入滚动状态时增加稳定 `threadId`、首次出现、最近推进、预期回收阶段和状态；规划下一批时优先读取到期未回收项，避免幼狼爪痕一类伏笔只被反复提及却没有闭合。
- [ ] 终局批次必须生成“必须回收 / 允许开放 / 已放弃并说明”清单；未处理的“必须回收”线索至少为 `warning`，用户明确要求闭合的线索为 `error`。
- [ ] 计划结果兑现检查按“已兑现 / 延迟到后场 / 未兑现 / 因用户修改而豁免”展示；不能只用关键词命中判断，也不能在结果已经语义兑现时误报。

阶段退出条件：

1. 对本次保留正文的离线分析可稳定复现约 17.32% 对话比例、过程标签和明显技术语体样本。
2. 新生成批次若要求 25%–35% 对话，界面能说明是否达标及哪些场景拉低比例，而不是只显示一句笼统评价。
3. 文风和伏笔检查可由用户关闭或调整；切换为科幻测试时不会误用黑童话技术词限制。
...
- F-09.6A—E 已实现并通过自动化；F-09.6F 的二十万字真实 DeepSeek 主链路已通过，但质量门禁未通过，当前不进入发布。
- F-09.6G 的统一严重度、正文清洁、边界重复、继续/完成/回流硬门禁、按场景修复与依赖失效，以及全局上下文逐阶段注入已实现；完整 `npm test` 和 `writer-audit` 已通过。下一步仍留在 F-09.6G，完成保存样本回归，再进入 F-09.6H。
- 2026-07-30 产品决策：长度只保持大致目标，不为精确字数牺牲质量；F-09.6H 优先把对话比例、技术说明腔、计划结果兑现等可见质量指标接入现有“锁”机制，并区分默认软锁与用户明确的硬锁。
- 测试项目必须保留原始正文供人工质量检查；所有自动修复都写新 Revision，不覆盖基线原文。
