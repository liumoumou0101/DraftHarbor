...
当前主线（2026-08-29）：场景摘要保存 **W-04 待验收**。写作一键排版 **W-03 待验收**。GitHub 下载者模型适配 **P-01 待验收**。设置页 UI **S-01 待验收**。恢复页 UI **R-01 待验收**。讨论区 UI **D-01 待验收**。资料库 UI **K-01 待验收**。写作工作区 UI **W-02 待验收**。
...
| 编号 | 功能 | 状态 | 依赖 | 完成标准 | 下一步 |
| --- | --- | --- | --- | --- | --- |
| W-03 | 写作区一键排版 | 待验收 | manuscript-format、writer-format、更多菜单/排版面板/右键 | 当前场景或选区可一键整理为网文段落：段首全角缩进、中文全角标点、去掉多余空行、硬换行合并；幂等；生成确认前不改稿；不改 `writer-core.js` | 2026-08-29：`node tests/manuscript-format.js`、`node tests/release-config.js`、`node tests/writer-button-audit.js` 应覆盖整理与幂等 |

### W-03 产品边界

- 这是正文整理，不是字号/行距面板。现有「排版」仍管稿纸外观。
- 默认网文格式：段首两个全角空格、段间不留空行、场景分隔行（如 `***`）不缩进。
- 有选区则只整理选区所在段落；否则整理当前场景。不提供全书排版。
- 生成进行中或待确认时拒绝排版，避免打乱镜像确认稿。
- 不改 AI 协议、确认条和 `writer-core.js`。

## 当前阶段：场景摘要保存（W-04）

| 编号 | 功能 | 状态 | 依赖 | 完成标准 | 下一步 |
| --- | --- | --- | --- | --- | --- |
| W-04 | 摘要弹窗可保存，并写入资料笔记 | 待验收 | writer-overlays、资料库 note、context-resolver | 生成后弹窗有「保存摘要」；写入场景/章节摘要字段；资料库「摘要」页 upsert 一张 note；默认提及时注入；标题用「第 1 章 · 第 1 场」 | 2026-08-29：`node tests/summary-workflow.js` 应覆盖弹窗保存、摘要页展示、mention 策略、场景/章节各一张笔记 |

### W-04 产品边界

- 下一场/下一章的前情默认用 `scene.summary` / `chapter.summary`，不是把每篇摘要设成始终注入的资料卡。
- 资料笔记放在资料库「摘要」页，不进「笔记」页；默认 `mention`（提及时注入），避免和场景摘要字段重复总是灌进 Prompt。
- 同一场景或章节再次保存只更新原笔记，不另开新卡。
- 不改 `writer-core.js`、AI 协议和确认条。

## 当前阶段：写作提示词装配打磨

| 编号 | 功能 | 状态 | 依赖 | 完成标准 | 下一步 |
| --- | --- | --- | --- | --- | --- |
| W-01 | 按拍停笔、模糊篇幅、模板收口与讨论接线 | 已完成 | Prompt 构建、内置模板、写作区、讨论区 | 正文 closer 不再用固定段数/具体字数；篇幅三档为 brief/natural/expanded；内置模板按拍职责精简；改写下拉与改写模板同一套源；讨论区可选内置角度并写入会话 | 2026-08-13：收尾两项与 8000 实测已完成，见下 |

### W-01 产品边界

- 停笔条件是「本拍写完」，不是「2–3 段」或「写到第 N 字」。
- 篇幅只提供模糊档；稿纸目标字数不进模型；`maxTokens` 仍是 API 天花板。
- 输入上下文预算（续写 6000、改写 ±1200、重生成 ±8000、摘要源约 18000）不是文学限长，本切片不砍。
- 不内置成人向专用模板；不把工作流目标字数改成硬截断。

### W-01 完成证据

- 2026-08-13：`prompt-builder` 默认 closer 改为写完本拍即停；`generationDefaults.lengthHint` 与写作区「篇幅」下拉接通。
- 内置模板：正文 13 / 改写 11 / 摘要 5 / 讨论 6 / 工作流 6；正文共用中文契约。清掉默认「2–4 段」及改写/摘要数字配额。
- 改写下拉由 `rewritePresetByKey` / `defaultPromptTemplates('rewrite')` 填充；讨论区「讨论角度」写入 `workshopSession.promptTemplateId`。
- 同日 Codex 复核后补修：风格快捷 `literary`、ESLint 全局表、`writer-core.js` 行数、摘要旧断言、closer「下一拍」措辞。定向单测 + `summary-workflow` 应通过。完整 `npm test` 以当时门禁为准。
- 真实 DeepSeek 对照每条件基本一次采样；墙钟差异不能直接写成思考 token 三倍。精简拼装空输出有混杂变量。
- 2026-08-13 夜已补 3 题材 × 3 档 × 3 次（27 次，思考开）：brief 中位 259 字且 9/9 本拍完整、0 越界；natural 中位 427；expanded 中位 340、均值 432，波动大。R18 同法 27 次中位 409 / 652 / 802。随后发现至少 3 次撞到 `maxTokens: 4000` 截断，旧脚本未记 `finishReason`，会高估「写完即停」。
- 2026-08-13 续：测试补录截断；默认输出额度 8000；思考开启且上限低于 8000 时写作区自动提到 8000 并提示。旧默认 2000 验收 18 次：有效完成 13、截断 5，另有 4 次首发正文为空后重试。
- 2026-08-13 收尾：截断时顶部状态合并「额度用尽 + 未保存」；阅读器触控/滚轮/滑动恢复默认门禁（连续 3 次通过）。窗边骑乘 8000 实测 6/6 完成、0 截断。
- 对照原始生成物保留在本机 `.ai_state`，不进仓库。详见 `docs/SESSION_HANDOFF.md` 2026-08-13 节。
...
## 规划阶段：Reader 2.0 专业小说阅读器

| 编号 | 功能 | 状态 | 依赖 | 完成标准 | 下一步 |
| --- | --- | --- | --- | --- | --- |
| F-12 | Reader 2.0 专业小说阅读器 | 进行中：F-12.1—F-12.12 数据主线、书库、详情、专注阅读壳、外观工作室、分页输入、导航、书签、批注、稿湾转交、用户字体、TTS/自动阅读、EPUB 与发布门禁已落地 | F-10 Reader Document/Locator/Store/Transfer、桌面设计系统、项目投影 | Reader 可以作为独立桌面小说阅读器长期使用；具备正式书库、专业排版、完整纸张/主题、分页/翻页、导航、书签、高亮、批注和字体扩展基础；选择后可安全转写作、资料库和工作流 | 评估 DOCX/PDF 等后续格式扩展 |

### F-12 产品边界

- Reader 是稿湾第三核心模块，仅次于写作与半自动工作流；首先必须是一款合格的独立小说阅读器。
- 保留 F-10 已验收的 Reader Document、稳定 Locator、Store、分页和 Transfer 数据基础。
- 新导入统一进入 Reader Store；旧 `draftharbor:desktop:reader` 只用于迁移。
- 普通阅读态不常驻 AI、写作或工作流动作；用户明确选择范围后才显示转交。
- 首期开发可只显示一种字体，但偏好必须保存稳定 `fontId`，并从第一阶段建立 Font Catalog/Provider、加载状态、回退和分页缓存接口。
- 后续用户字体优先安装到应用管理目录，不要求修改操作系统字体；增加字体不能重构 Reader Document、Locator 或偏好主结构。
- 正式设计：`docs/READING_MODULE_2_0_DESIGN.md`。
- 开发计划：`docs/READING_MODULE_2_0_DEVELOPMENT_PLAN.md`。

### F-12 建议阶段

- [x] F-12.0：当前证据、状态图、三个完整视觉方向、选定视觉基线，以及 Reader 模块地图和体积门禁（F-12.0D 结构与体积门禁已落地；视觉方向冻结仍需后续视觉验收）。
- [x] F-12.1：新版导入向导、项目/外部书库聚合和 legacy localStorage 收口（导入字节预览/确认、校正、取消、Reader Store 权威流及项目只读投影聚合已接通；旧键只承担可验证迁移）。
- [x] F-12.2：Preferences v2、阅读方案、Theme V1、字体目录/Provider、批注和历史契约（正式偏好存储与迁移已升级；批注/历史 Store、API 和 UI 已接通；用户字体文件管理已在 F-12.10 完成）。
- [x] F-12.3：正式书库首页、继续阅读、项目作品、本地书籍和书籍详情（聚合摘要、筛选/排序、收藏、自定义书架、移出书架、重新导入和详情目录预览已接通；安全删除与引用检查留待后续存储治理）。
- [x] F-12.4：专注阅读壳、HUD、全局 chrome 协调、焦点状态和样式拆分（五态 HUD、自动隐藏/输入恢复、对话框/面板/选区优先级、全局导航与标题栏收起/恢复、隐藏控件焦点转移已接通；完整主题/状态栏字段选择继续留在 F-12.5/F-12.6）。
- [x] F-12.5：快捷外观、外观工作室、六个完整主题、纸张材质和单字体多字体架构演练（六主题 token、快捷外观条、六分区工作室、实时预览、全局/单书作用域、用户方案保存/删除/冲突保护、本地细纹资产及字体 Provider 回退均已接通；详见 `docs/READER_ASSET_LICENSES.md`）。
- [x] F-12.6：中文分页质量、翻页输入、动效 Adapter、状态栏与进度（中文标点/中英混排/emoji 安全分页、孤行/寡行控制、字重/书脊/字体版本缓存键、键鼠滚轮/触控输入协调、fade/slide/cover/none Adapter、状态栏字段与自动隐藏均已接通；桌面回归已覆盖选区/面板/对话框误翻抑制）。
- [x] F-12.7（2026-08-04）：导航中心、书签升级、高亮、批注和位置历史（新增批注/历史面板、选区高亮/下划线/批注弹窗、Range 恢复精度、批注增删改、书签颜色/分类/备注/最近访问、搜索/目录/书签/批注导航历史和 100 条有界存储；桌面回归已覆盖跨布局选择、持久化和失败恢复）。
- [x] F-12.8（2026-08-04）：选择模式与写作、资料库、工作流转交（普通阅读保持低噪声；选择模式支持文本/场景/章节/多章/全文范围，并提供高亮、批注、书签、复制和“发送到稿湾”；Writer、Compendium、Workflow 均复用不可变 Transfer Envelope，支持预览/应用、fresh/stale/missing 和新 Revision 提示；项目稿源可返回 Reader 并恢复项目投影、Revision 与 Locator）。
- [x] F-12.9（2026-08-04）：自动化、视觉 QA、无障碍、性能、恢复、构建和安装版发布验收（新增统一 `reader-quality-acceptance` 入口、Reader 无障碍/键盘焦点验收、四档缩放与多视口视觉审计；百万字性能、12 组来源/目标矩阵、压力 Envelope、unpacked/NSIS/Portable 和安装版冒烟均已通过；壳层结构门禁和全局 unit/桌面主线回归通过）。
...
| 编号 | 问题 | 影响 | 当前判断 | 后续处理 |
| --- | --- | --- | --- | --- |
| DH-STARTUP-001 | 清理进程前，首次启动偶发出现数秒延迟或一闪而过；诊断中观察到残留/重复 Electron 进程以及 Chromium cache/GPU profile 锁定报错 | 影响启动稳定性和用户对新版本 Reader 的第一印象；目前无法稳定复现为功能代码阻塞 | 已确认更像 Electron 多实例、缓存或 GPU 初始化状态问题，而非 Reader 新脚本同步 I/O；清理残留进程后连续启动恢复正常 | 后续单独在主进程增加单实例锁、二次实例聚焦已有窗口，并补充冷启动/缓存锁定回归；在此问题关闭前保留诊断日志，不把它误判为 Reader 功能完成 |
