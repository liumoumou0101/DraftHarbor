...
最后更新：2026-07-29。此文件是 DraftHarbor 后续会话的入口；功能状态以当前仓库、`docs/FEATURE_TODO.md`、对应设计/验收文档与本文件共同为准。
...
- 2026-07-29：首次完整跑通后的产品反馈已整理为 `docs/WORKFLOW_FIRST_FULL_RUN_FEEDBACK_2026-07-29.md`。下一阶段最高优先级不是整体 UI 重做，而是：可由用户确认驱动的多批次持续生成、全局写作指令、每步自然语言修改意见，以及正文场景间的滚动上下文。
...
1. 设计并实现用户确认驱动的多批次持续生成，显示累计字数、目标字数和下一批范围。
2. 给工作流增加冻结且可版本化的全局写作指令，明确它作用于哪些生成阶段。
3. 正文由“预先准备全部独立 Prompt”改为顺序生成；每场完成后更新滚动摘要、人物状态、未解线索和上一场结尾，再准备下一场。
4. 每个待确认步骤增加自然语言修改意见，支持局部/整体 AI 重写并产生新 Revision。
5. 然后处理生成/校验/修复/写入的进度展示、历史 Revision 选择和完整阅读/表单/JSON 三视图。
6. 按钮统一和整体 UI 重排暂列低优先级。
